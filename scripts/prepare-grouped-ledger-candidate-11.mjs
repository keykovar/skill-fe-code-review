...
function usage() {
  return 'Usage: node scripts/prepare-grouped-ledger-candidate-11.mjs --output <directory> --cursor-executable <absolute-file> --git-executable <absolute-file> --skill-source <directory>';
}
...
function buildPrompt(prepared, runDefinition, evidence) {
  const testEvidence =
  ...
  const common =
    `${testEvidence} 仅可使用 workspace 内的只读文件工具读取已识别的变更、基线、直接调用方和测试；不得读取 workspace 外路径、Cursor 会话/日志、oracle、evaluation、examples/outputs 或历史 trace。不得调用 Shell、MCP、写入、编辑、删除、计划写入或其他非读取工具，不得修改文件、Git 状态、分支或配置。任何工具失败或拒绝都不得重试；请保留证据边界并输出最终 Review。`;
...
function prepareRun(runDefinition, directories, skillSource, gitExecutable) {
  ...
  const prompt = buildPrompt(prepared, runDefinition, evidence);
...
export function prepareCandidate11(requestedOutput, requestedCursorExecutable, requestedGitExecutable, requestedSkillSource) {
  ...
  const preparedRuns = runs.map((runDefinition) =>
    prepareRun(runDefinition, directories, skillSource, gitExecutable),
  );
  const manifest = {
    schemaVersion: 1,
    ...
    predecessor: {
      candidate: 'post-v0.4.0-grouped-ledger-candidate-10',
      decision: 'No-Go',
      reason: 'Plan-mode Shell was rejected before execution; Candidate 11 moves inventory, collection, and tests to the evaluator.',
      retried: false,
      reinterpreted: false,
    },
    outputDir,
    ...
    preparedRuns,
    repeatAssertions: [repeatAssertion(preparedRuns, 'Q-ID-001'), repeatAssertion(preparedRuns, 'F-ID-001')],
    sourcePolicy: {
