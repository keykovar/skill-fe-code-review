...
export const candidate06ArgumentsBeforeWorkspace = [
  '--print',
  '--output-format',
  'stream-json',
  '--mode',
...
export const candidate06ForbiddenFlags = [
  '--auto-review',
  '--force',
  '--yolo',
  '--approve-mcps',
...
export function hashCursorEvaluationTreeV3(directory) {
...
export function runCursorEvaluationV3(argv, environment = process.env) {
  ...
  const clientArguments = [...candidate06ArgumentsBeforeWorkspace, workspace, prompt];
  if (candidate06ForbiddenFlags.some((flag) => clientArguments.includes(flag))) {
    throw new Error('Candidate 06 client arguments contain a forbidden permission flag.');
  }
...
function main() {
  try {
    const summary = runCursorEvaluationV3(process.argv.slice(2));
