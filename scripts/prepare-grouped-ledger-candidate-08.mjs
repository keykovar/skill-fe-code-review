...
import { buildGroupedLedgerCandidate08 } from './build-grouped-ledger-candidate-08.mjs';
import { hashCursorEvaluationTreeV4 } from './run-cursor-evaluation-v4.mjs';
...
function usage() {
  return 'Usage: node scripts/prepare-grouped-ledger-candidate-08.mjs --output <directory> --cursor-executable <absolute-file> --git-executable <absolute-file>';
}
...
function promptFor(basePrompt, mode, testEvidence) {
  const suppliedTestEvidence =
    `离线评测器已在冻结前执行一次声明的确定性测试 \`node --test\`，实际退出码为 ${testEvidence.exitCode}。` +
    '将其作为提供的本地测试证据；完整 stdout/stderr 哈希仅保存在本地冻结清单中，不发送给模型；不得在 Cursor 中再次执行任何测试命令。';
  const common =
    `${suppliedTestEvidence} 可按需读取已识别的变更文件、对应基线、直接调用方和测试，但不得读取工作区外路径、Cursor 会话/日志、oracle、evaluation、examples/outputs 或历史 trace。不得调用写入、编辑、删除、计划写入或 MCP 工具，不得修改任何文件、Git 状态、分支或配置。任何命令失败或被拒绝都不得重试；请保留证据边界并输出最终 Review。`;
...
function prepareWorkspace(runDefinition, directories, candidate, gitExecutable) {
  ...
  const prompt = promptFor(prepared.prompt, runDefinition.mode, testEvidence);
  ...
  const agentSkillTree = hashCursorEvaluationTreeV4(
    path.join(workspace, '.agents', 'skills', 'fe-code-review'),
  );
  const cursorSkillTree = hashCursorEvaluationTreeV4(
...
export function prepareCandidate08(
  requestedOutput,
  ...
  const candidate = buildGroupedLedgerCandidate08(directories.candidate);
  ...
  const manifest = {
    ...
    predecessor: {
      ...
      decision: 'No-Go',
      reason: 'Ask mode denied read-only Shell inventory and repeated test execution, while ledger child locations used ranges or omitted line numbers.',
      retried: false,
      reinterpreted: false,
    },
    outputDir,
    ...
    promptContract: {
      ...
      deterministicTest: 'executed once by the offline evaluator; exit code supplied to Cursor, stdout/stderr SHA-256 retained only in the local manifest, expected Cursor test calls 0',
      transport: 'one literal final Prompt argv value',
    },
    generationBoundary: {
      source: 'Candidate 08 deterministic offline builder and fresh fixture preparation',
