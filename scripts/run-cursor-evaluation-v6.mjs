...
const requiredOptions = [
  ...
  '--expected-seatbelt-profile-sha256',
...
export const candidate10ArgumentsBeforeWorkspace = [
  '--print',
  '--output-format',
  'stream-json',
  '--mode',
...
export const candidate10ForbiddenFlags = [
  '--auto-review',
  '--force',
  '--yolo',
  '--approve-mcps',
...
export function hashCursorEvaluationTreeV6(directory) {
...
export function runCursorEvaluationV6(argv, environment = process.env) {
  ...
  const sandboxExecutable = requireAbsoluteFile(
    options['--sandbox-executable'],
    '--sandbox-executable',
    true,
  );
  const seatbeltProfile = requireAbsoluteFile(
    options['--seatbelt-profile'],
    '--seatbelt-profile',
  );
  const home = requireAbsoluteDirectory(options['--home'], '--home');
  const protectedRoot = requireAbsoluteDirectory(
    options['--protected-root'],
    '--protected-root',
  );
  const promptFile = requireAbsoluteFile(options['--prompt-file'], '--prompt-file');
  if (
    isInside(workspace, home) ||
    isInside(protectedRoot, home) ||
    isInside(workspace, promptFile) ||
    isInside(workspace, seatbeltProfile)
  ) {
    throw new Error(
      'Cursor HOME must stay outside the reviewed workspace and protected root; Prompt and Seatbelt profile must stay outside the reviewed workspace.',
    );
  }
  ...
  const seatbeltProfileSha256 = sha256(fs.readFileSync(seatbeltProfile));
  requireEqual(
    seatbeltProfileSha256,
    requireSha256(
      options['--expected-seatbelt-profile-sha256'],
      '--expected-seatbelt-profile-sha256',
    ),
  ...
  const skillTree = hashCursorEvaluationTreeV6(skillDirectory);
  requireEqual(
    skillTree.sha256,
  ...
  const clientArguments = [...candidate10ArgumentsBeforeWorkspace, workspace, prompt];
  if (candidate10ForbiddenFlags.some((flag) => clientArguments.includes(flag))) {
    throw new Error('Candidate 10 client arguments contain a forbidden permission flag.');
  }
  ...
  const result = spawnSync(sandboxExecutable, sandboxArguments, {
    cwd: workspace,
    encoding: 'utf8',
    env: {
      ...environment,
  ...
  const postStatus = runChecked(
    gitExecutable,
    ['status', '--short', '--untracked-files=all'],
    workspace,
    'Post-run Git status',
  );
  requireEqual(sha256(postStatus), statusSha256, 'Post-run Git status SHA-256');
  const postWorkspaceTree = hashCursorEvaluationTreeV6(workspace);
  requireEqual(
    postWorkspaceTree.sha256,
    workspaceTree.sha256,
    'Post-run workspace tree SHA-256',
  );
  ...
  return {
    ...
    postflight: {
      statusSha256: sha256(postStatus),
      workspaceTreeSha256: postWorkspaceTree.sha256,
    },
    preflight: {
      branch,
      clientVersion,
    ...
    promptSha256,
    sandboxArgumentsBeforeClient: sandboxArguments.slice(0, 6),
...
function main() {
  try {
    const summary = runCursorEvaluationV6(process.argv.slice(2));
