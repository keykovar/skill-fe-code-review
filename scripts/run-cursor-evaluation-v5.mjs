...
export const candidate09ArgumentsBeforeWorkspace = [
  '--print',
  '--output-format',
  'stream-json',
  '--mode',
...
export const candidate09ForbiddenFlags = [
  '--auto-review',
  '--force',
  '--yolo',
  '--approve-mcps',
...
export function hashCursorEvaluationTreeV5(directory) {
...
export function runCursorEvaluationV5(argv, environment = process.env) {
  ...
  const clientArguments = [...candidate09ArgumentsBeforeWorkspace, workspace, prompt];
  if (candidate09ForbiddenFlags.some((flag) => clientArguments.includes(flag))) {
    throw new Error('Candidate 09 client arguments contain a forbidden permission flag.');
  }
  ...
  const postWorkspaceTree = hashCursorEvaluationTreeV5(workspace);
  requireEqual(
...
function main() {
  try {
    const summary = runCursorEvaluationV5(process.argv.slice(2));
