...
export function auditCursorPlanProbe(argv) {
  ...
  const completedCommand = completedShell?.result?.success?.command;
  if (startedCommand !== expectedCommand || completedCommand !== expectedCommand) {
    violations.push({ actual: `${startedCommand ?? ''}/${completedCommand ?? ''}`, expected: expectedCommand, type: 'command-mismatch' });
  }
  ...
  const commandResult = completedShell?.result?.success;
  ...
  return {
    ...
    commandExecutionCount: completedShell ? 1 : 0,
