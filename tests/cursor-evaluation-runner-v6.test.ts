...
import {
  candidate10ForbiddenFlags,
  hashCursorEvaluationTreeV6,
} from '../scripts/run-cursor-evaluation-v6.mjs';
import { rootDir } from './test-utils';

const temporaryDirectories: string[] = [];
const runnerPath = path.join(rootDir, 'scripts', 'run-cursor-evaluation-v6.mjs');
...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'fe-cursor-runner-v6-'));
  ...
  fs.writeFileSync(path.join(skillDirectory, 'SKILL.md'), '# Frozen Candidate 10 Skill\n');
  ...
  const args = [
    ...
    '--expected-client-version', 'fake-cursor-v6',
...
describe('Cursor evaluation runner v6 Candidate 10 contract', () => {
  test('wraps exact plan-mode arguments in the frozen Seatbelt profile', () => {
    ...
    expect(capture.argv).toEqual([
      '--print', '--output-format', 'stream-json', '--mode', 'plan', '--sandbox', 'disabled',
      '--trust', '--workspace', fs.realpathSync(fixture.workspace), fixture.prompt,
    ]);
    expect(candidate10ForbiddenFlags.every((flag) => !capture.argv.includes(flag))).toBe(true);
