...
import { hashCursorEvaluationTreeV6 } from '../scripts/run-cursor-evaluation-v6.mjs';
import { rootDir } from './test-utils';

const temporaryDirectories: string[] = [];
const auditPath = path.join(rootDir, 'scripts', 'audit-cursor-plan-probe-v2.mjs');
...
function createFixture() {
  ...
  const workspaceTreeSha256 = hashCursorEvaluationTreeV6(workspace).sha256;
  ...
  const treeWithTraceSha256 = hashCursorEvaluationTreeV6(workspace).sha256;
...
describe('Candidate 10 Cursor Plan-mode probe audit v2', () => {
  ...
  test('rejects a result event that is not the ordered assistant aggregate', () => {
    ...
    fixture.treeWithTraceSha256 = hashCursorEvaluationTreeV6(fixture.workspace).sha256;
    const result = audit(fixture);
  ...
  test('rejects a failed command while preserving its actual command and exit code', () => {
    const fixture = createFixture();
    const completed = fixture.events.find(
      (event) => event.type === 'tool_call' && event.subtype === 'completed',
    )!;
    completed.tool_call!.shellToolCall!.result = {
      failure: {
        command,
        exitCode: 71,
        stderr: 'sandbox application denied\n',
        stdout: '',
        workingDirectory: '',
      },
    };
    ...
    expect(output.violations).toEqual(
      expect.arrayContaining([
        { actual: 71, type: 'command-failed' },
        { actual: 71, expected: 0, type: 'command-exit' },
        { type: 'command-output-not-empty' },
      ]),
    );
    expect(output.violations).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ type: 'command-mismatch' })]),
    );
  });
});
